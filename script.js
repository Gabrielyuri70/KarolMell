...
function sincronizarDados() {
    ...
    // Adicione esta linha para carregar as categorias também
    const categorias = JSON.parse(localStorage.getItem('meu_catalogo_categorias')) || [];
...
function gerarCardHTML(p, index) {
    ...
    // Lógica da TAG personalizada
    let textoTag = "";
    if (p.tag && p.tag.trim() !== "") {
        textoTag = p.tag.toUpperCase();
    } else if (temPromo) {
        textoTag = "PROMOÇÃO";
    }
